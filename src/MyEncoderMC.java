...
public class MyEncoderMC {
	...
	public static void main(String[] args) {
		...
		int gopSize = 3; //GOP size number of frames between 2 I-frames
		int width = 480;
		int height = 368;
		...
		int numberOfIFrames = numberOfFrames/gopSize;
		int[][] quantizedData = new int[numberOfIFrames][];//store I-frames only

		for(int index = 0; index <= numberOfIFrames - 1; index++){
			byte[] img = new byte[frameDataSize];
			System.arraycopy(bytes, index*gopSize*frameDataSize, img, 0, frameDataSize); //copy I-frame to byte array img
			boolean[][] layerOfImg8 = convert16To8(layerOfVideo16[index*gopSize]); //convert layer info to 8x8
			quantizedData[index] = LayeredImageCodec.encoder420z(img, width, height, n1, n2, layerOfImg8); //DCT & quantize
		}
		
		time = System.currentTimeMillis() - time;
		System.out.println("Total time of Motion Compensation: " + (time/1000) + " seconds");
		
		//write data to output file
		try {
			writeToFile(outputFile, width, height, n1, n2, vectorsOfVideo, layerOfVideo16, quantizedData, MC);
		} catch (IOException e) {
		...
		window.setTitle("Display Window: MyEncoderMC");
